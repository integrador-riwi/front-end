...
export default class QRVoting {
  ...
  updateQrButtonState() {
    ...
    if (this.qrActive) {
      btn.disabled = false;
      btn.innerText = t("voting.disableQr") || "Disable QR";
    ...
    if (canGenerate) {
      ...
      btn.title = !this.finalistsApproved
          ? t("voting.approveFirst") || "Approve finalists first"
          : t("voting.selectExpFirst") || "Select an expiration date first";
    }
  }
    ...
    btn.addEventListener("click", async () => {
      try {
        if (this.qrActive) {
          ...
          btn.disabled = true;
          btn.innerHTML = `<span class="spinner-border spinner-border-sm me-2"></span>${t("voting.disabling") || "Disabling..."}`;
          ...
          btn.disabled = false;
          btn.innerText = t("voting.generateQr") || "Generate QR";
        ...
        btn.innerHTML = `
          ...
          ${t("voting.generating") || "Generating QR..."}
        `;
  ...
  async handleSubmitVotes() {
    ...
    btn.addEventListener("click", async () => {
      ...
      btn.innerHTML = `
      <span class="spinner-border spinner-border-sm me-2"></span>
      ${t("voting.calcWinners") || "Calculating winners..."}
    `;
      ...
      try {
        ...
        btn.innerHTML = `<span class="icon-md align-items-center">${icons.checked()}</span> ${t("voting.winnersCalculated") || "Winners calculated!"}`;
        ...
        <div class="d-flex align-items-center gap-3 p-2">
          ...
          <div>
            <div class="fw-bold" style="color:#5acca4;">${t("voting.votingClosed") || "Voting closed successfully!"}</div>
            <div style="color:#7b7fa8;font-size:0.85rem;">${t("voting.winnersCalculatedGo") || "Winners have been calculated. Go to the Finalists page to see the podium."}</div>
          </div>
        </div>
      `;
      } catch (err) {
        ...
        if (err?.response?.status === 409 || err?.message?.includes("once")) {
          ...
          alert(t("voting.errorCalcWinners") + err.message);
          btn.disabled = false;
          btn.innerHTML = t("voting.submitVotesAndCalc") || `Submit Votes & Calculate Winners`;
        }
      }
    });
  }
    ...
    container.innerHTML = `
      <div class="rounded-4 p-4" style="background:#fff;border:1.5px solid rgba(107,92,255,0.15);">
        <div class="d-flex align-items-center gap-2 mb-3">
          ...
          <h3 class="fw-bold mb-0" style="color:#181e4b;font-size:1rem;">${t("voting.staffVotingLink") || "Staff Voting Link"}</h3>
          <span class="badge rounded-pill ms-1" style="background:rgba(107,92,255,0.1);color:#6b5cff;font-size:0.7rem;">${t("voting.private") || "PRIVATE"}</span>
        </div>
        <p class="mb-3" style="color:#7b7fa8;font-size:0.83rem;">
          ${t("voting.staffVotingDesc") || "Generate a private link for staff members only. This link is different from the public QR and won't be visible to the audience."}
        </p>
    ...
    if (copyBtn) {
      copyBtn.addEventListener("click", async () => {
        try {
          ...
          copyBtn.textContent = t("voting.copied") || "Copied!";
          copyBtn.style.background = "rgba(90,204,164,0.15)";
          copyBtn.style.color = "#059669";
          setTimeout(() => {
            copyBtn.textContent = t("voting.copy") || "Copy";
  ...
  handleResetVotes() {
    ...
    btn.addEventListener("click", async () => {
      const confirmed = confirm(
          "WARNING: " + (t("voting.resetWarn") || "This will permanently delete ALL votes for this event. Are you sure?")
      );
      if (!confirmed) return;
    ...
    `;
      ...
      try {
        ...
        btn.innerHTML = t("voting.votesResetSuccess") || `Votes reset successfully`;
        btn.disabled  = false;
  ...
  renderRankingPanel() {
    ...
    <div class="d-flex align-items-center justify-content-between mb-4">
      <div>
        <h2 class="fw-bold mb-1" style="color:#181e4b;font-size:1.1rem;">${t("voting.eventRanking") || "Event Ranking"}</h2>
        <p class="mb-0" style="color:#7b7fa8;font-size:0.85rem;">${t("voting.topTeams") || "Top performing teams"}</p>
      </div>
      ...
      ${this.finalistsApproved ? `
        <div class="d-flex align-items-center gap-2 rounded-3 px-3 py-2"
          ...
          <span style="color:#059669;font-size:0.85rem;font-weight:700;">
            ${this.finalists.length} ${t("voting.finalistsApproved") || "finalists approved"}
          </span>
        </div>` : `<div></div>`}
      <button class="btn btn-approve fw-bold px-4 py-2 rounded-3" id="approve-finalists-btn"
              ${this.finalistsApproved ? "disabled" : ""}>
        ${this.finalistsApproved ? (t("voting.finalistsApprovedBtn") || "Finalists Approved") : (t("voting.approveFinalistsBtn") || "Approve Finalists")}
      </button>
    </div>
  `;
  ...
  renderResults() {
    ...
    const medals = [
      `<svg viewBox="0 0 24 24" fill="none" stroke="#d4af37" stroke-width="2" width="20" height="20">
         <path d="M8 21h8M12 17v4M7 4h10v5a5 5 0 0 1-10 0V4zM5 6h2v2a3 3 0 0 1-3-3V4h3M19 6h-2v2a3 3 0 0 0 3-3V4h-3"/>
       </svg>`,
      `<svg viewBox="0 0 24 24" fill="none" stroke="#c0c0c0" stroke-width="2" width="20" height="20">
         <path d="M8 21h8M12 17v4M7 4h10v5a5 5 0 0 1-10 0V4zM5 6h2v2a3 3 0 0 1-3-3V4h3M19 6h-2v2a3 3 0 0 0 3-3V4h-3"/>
       </svg>`,
      `<svg viewBox="0 0 24 24" fill="none" stroke="#cd7f32" stroke-width="2" width="20" height="20">
         <path d="M8 21h8M12 17v4M7 4h10v5a5 5 0 0 1-10 0V4zM5 6h2v2a3 3 0 0 1-3-3V4h3M19 6h-2v2a3 3 0 0 0 3-3V4h-3"/>
       </svg>`
    ];
    ...
    modal.innerHTML = `
      <div style="background:#fff;border-radius:16px;width:100%;max-width:780px;max-height:85vh;display:flex;flex-direction:column;overflow:hidden;box-shadow:0 20px 60px rgba(0,0,0,0.25);">
        <div style="padding:20px 24px;border-bottom:1px solid #e5e7eb;display:flex;justify-content:space-between;align-items:center;">
          <h5 style="margin:0;font-weight:700;font-size:1.1rem;display:flex;align-items:center;gap:8px;">
    ...
    try {
      ...
      const invalidAlert = summary.invalid > 0
          ? `<div style="background:#fef2f2;border:1.5px solid #fca5a5;border-radius:10px;padding:12px 16px;margin-bottom:16px;color:#991b1b;font-size:.875rem;">
             <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="16" height="16" style="vertical-align:text-bottom;margin-right:4px;">
               <path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"></path><line x1="12" y1="9" x2="12" y2="13"></line><line x1="12" y1="17" x2="12.01" y2="17"></line>
             </svg>
             <strong>${summary.invalid} voto(s) con firma inválida detectados.</strong>
      ...
      const noHashAlert = summary.no_hash > 0
          ? `<div style="background:#fffbeb;border:1.5px solid #fcd34d;border-radius:10px;padding:12px 16px;margin-bottom:16px;color:#92400e;font-size:.875rem;">
             ...
             <strong>${summary.no_hash} voto(s) sin firma HMAC.</strong>
