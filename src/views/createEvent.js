...
export default class CreateEvent {
  ...
  _getEventData() {
    return {
      ...
      githubOrg:
        document.getElementById("ev-github-org")?.value?.trim() || this.selectedGithubOrg || null,
  ...
  _validate() {
    ...
    if (!ev.title) return t("createEvent.validTitle") ?? "Event title is required.";
    if (!ev.eventDate) return t("createEvent.validDate") ?? "Start date is required.";
  ...
  async _handleSubmit() {
    ...
    try {
      ...
      toast.success(
        ...
        t("createEvent.successMsg") ?? "The event has been created successfully.",
      );
      setTimeout(() => this.router.navigate("events"), 1600);
    } catch (e) {
      toast.error(t("common.errorTitle"), e.message ?? t("createEvent.errorCreating"));
    } finally {
  ...
  async _handleUpdate() {
    ...
    try {
      ...
      toast.success(
        ...
        t("createEvent.updateSuccessMsg") ?? "The event has been updated successfully.",
      );
  ...
  async _syncRubrics() {
    ...
    if (toDelete.length > 0) {
      await Promise.all(toDelete.map((id) => deleteEventRubric(this.eventId, id)));
    }
    ...
    if (toCreate.length > 0) {
      const rubrics = toCreate.map(({ area, crit }) => this._buildRubricPayload(area, crit));
  ...
  async _loadEditData() {
    ...
    try {
      ...
      try {
        ...
        this._loadedRubrics =
          rubricsRes?.data?.rubrics ?? rubricsRes?.rubrics ?? rubricsRes?.data ?? rubricsRes ?? [];
        this._applyRubricsForEdit(this._loadedRubrics);
      } catch (rubricErr) {
        toast.warning(
          t("common.errorTitle"),
  ...
  _applyEventToForm(event) {
    ...
    const endDate = event.end_date ?? event.final_delivery_date ?? event.endDate ?? null;
  ...
  _downloadTemplate() {
    ...
    toast.success(t("createEvent.templateDownloadTitle"), t("createEvent.templateDownloadMsg"));
  }
        ...
        for (const sn of workbook.SheetNames) {
          ...
          for (let i = 0; i < Math.min(safeRows.length, 15); i++) {
            ...
            if (row.includes("area") && row.includes("criterio")) {
              this._parseExcelRubric(row, safeRows.slice(i + 1));
              return;
            }
          }
        }
        toast.error(t("createEvent.formatErrorTitle"), t("createEvent.missingRubricColumns"));
      } catch (err) {
        toast.error(t("createEvent.importErrorTitle"), t("createEvent.importProcessError"));
      }
    };
  ...
  _parseExcelRubric(headerRow, rows) {
    ...
    if (missingCols.length > 0) {
      toast.error(
        ...
        t("createEvent.missingColumns", { columns: missingCols.join(", ") }),
      );
      return;
    }
    ...
    rows.forEach((row, rowIndex) => {
      ...
      levels.forEach((lvl) => {
        if (!lvl.description)
          errors.push(`Row ${rowNum}: Missing description for level '${lvl.name}'.`);
      });
  ...
  async _loadGithubOrgs() {
    ...
    try {
      ...
      this.selectedGithubOrg =
        preferredOrg && this.githubOrgs.some((org) => org.login === preferredOrg)
          ? preferredOrg
      ...
      toast.warning(t("createEvent.githubRequiredTitle"), t("createEvent.githubRequiredMsg"));

      if (submitBtn) {
        submitBtn.disabled = true;
        submitBtn.title = t("createEvent.connectGithub") ?? "Connect GitHub first";
      }
    }
  }
