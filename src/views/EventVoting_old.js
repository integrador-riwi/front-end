...
export default class QRVoting {
  ...
  renderTeamCard(team, { selectable = false } = {}) {
    ...
    <div class="col-md-6 col-lg-4">
      <div class="teamF-card ${selectable ? "selectable-team" : ""} 
           ...
           ${selectable ? `data-id="${team.id_team}"` : ""}>
  ...
  renderAvailableTeams(finalists = []) {
    ...
    const rankedIds = this.ranking.filter(Boolean).map((team) => team.id_team);


    const available = finalists.filter((team) => !rankedIds.includes(team.id_team));
  ...
  attachTeamSelection() {
    document.querySelectorAll(".selectable-team").forEach((card) => {
      card.addEventListener("click", (e) => {
        ...
        const team = this.finalists.find((t) => t.id_team == id);

        if (!team) return;

        // Toggle logic: if already ranked, remove it
        const rankedIndex = this.ranking.findIndex(
          (t) => t?.id === team.id_team,
        );
        if (rankedIndex !== -1) {
